    ...
    const DevvitBridge = {
        init: async () => {
            ...
            try {
                ...
                if (data.dbData) {
                    // Smart Merge: Merge server data into local, but let local changes take precedence 
                    // (prevents overwriting optimistic updates made during startup race conditions)
                    const serverData = data.dbData;
                    for (const col in serverData) {
                        if (window._genericDB[col]) {
                            window._genericDB[col] = { ...serverData[col], ...window._genericDB[col] };
                        } else {
                            window._genericDB[col] = serverData[col];
                        }
                    }
                    ...
                    // Dispatch Ready
                    window.dispatchEvent(new CustomEvent('GAMEDATA_READY', { detail: window._genericDB }));
                }
            } catch (e) { 
                console.warn("[Bridge] Init failed (might be offline)", e);
                // Fallback to Guest to unblock socket.initialize() waiting for identity
    ...
    window.GenericDB = {
        ...
        getAdapter: (name) => ({
             ...
             create: async (d) => {
                 ...
                 // Wait briefly for identity if missing (prevents anonymous scores on fast startup)
                 if (!window._currentUser) {
                     let tries = 0;
                     while(!window._currentUser && tries < 10) { 
